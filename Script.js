...
function renderHero() {
  ...
  document.getElementById('hero-bg').style.backgroundImage = `url('${movie.heroImage}')`;
}
...
function openCinemaModal(cinemaId) {
  ...
  const movies = cinema.movieIds.map(id => MOVIES[id]);
...
function openMovieModal(movieId, preselectedCinemaId = null) {
  ...
  const availCinemas = CINEMAS.filter(c => c.movieIds.includes(movieId));
...
function addToWatchlist(movieId) {
  ...
  if (state.watchlist.includes(movieId)) {
    showToast(`"${MOVIES[movieId].title}" est déjà dans votre Watchlist.`);
    return;
  }
  state.watchlist.push(movieId);
  saveState();
  showToast(`✓ "${MOVIES[movieId].title}" ajouté à la Watchlist`);
}
...
function rateWatchlistMovie(movieId, rating) {
  ...
  saveState();
  renderWatchlistPage();
  showToast(`★ Note ${rating}/5 enregistrée pour "${MOVIES[movieId].title}"`);
}
